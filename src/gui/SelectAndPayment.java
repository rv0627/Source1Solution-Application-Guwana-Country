 ...
 * @author Admin
 */
public class SelectAndPayment extends javax.swing.JDialog {
    ...
    public SelectAndPayment(java.awt.Frame parent, boolean modal, HashMap<String, String> dueMap) {
        super(parent, modal);
        initComponents();

        this.setTitle("Source 1 Solutions - Due payment");
        System.out.println(dueMap.get("paidAmount"));

        mi = (MainInterface) parent;

        if (mi.duePaymentRow != -1) {
            for (PaymentDetail value : mi.paymentDetailMap.values()) {
                jTextField1.setText(value.getWirNo());
        ...
        try {
            ...
            if (mi.duePaymentRow != -1) {
                ...
                if (jFormattedTextField2.getText().isEmpty()) {
                    ...
                    for (PaymentDetail value : mi.paymentDetailMap.values()) {
                        updatePayment = Double.parseDouble(value.getDueAmount());
                        MySQL.execute("UPDATE `invoice` SET"
                            + "`paid_ammount`=`paid_ammount`+'" + updatePayment + "' WHERE `invoice_id`='" + value.getWirNo() + "'");
                ...
                if (jFormattedTextField2.getText().isEmpty()) {
                    ...
                    for (PaymentDetail value : mi.paymentDetailMap.values()) {
                        ...
                        MySQL.execute("UPDATE `excelsheetdata` SET"
                                + "`paidAmount`=`paidAmount`+'" + updatePayment + "' WHERE `number`='" + value.getWirNo() + "'");
